...
void ScanMatcher::computeActiveArea(ScanMatcherMap& map, const OrientedPoint& p, const double* readings){
	...
	lp.theta+=m_laserPose.theta;//雷达在世界坐标的角度
	IntPoint p0=map.world2map(lp);//机器人坐标由世界坐标转地图坐标----机器人坐标-----点云line起点
	const double * angle=m_laserAngles;//激光桢的起始角度
	for (const double* r=readings; r<readings+m_laserBeams; r++, angle++)//r是指针,指向readings首位地址
	// readings+ ams表示最后的地址
		if (m_generateMap){//如果生成地图
			double d=*r;//距离值
			if (d>m_laserMaxRange)//超过最大范围,丢弃
				continue;
			if (d>m_usableRange)//超过有效范围,设为有效距离
				d=m_usableRange;
			//雷达坐标结合点云角度和长度,计算出点云世界坐标
			Point phit=lp+Point(d*cos(lp.theta+*angle),d*sin(lp.theta+*angle));
			IntPoint p1=map.world2map(phit);//点云世界坐标转地图栅格坐标------hit坐标---点云line终点
			
			d+=map.getDelta();//获取地图分辨率
			//Point phit2=lp+Point(d*cos(lp.theta+*angle),d*sin(lp.theta+*angle));
			//IntPoint p2=map.world2map(phit2);
			IntPoint linePoints[20000] ;//设置点云容量
			GridLineTraversalLine line;//创建点云直线容器,存储激光束上所有点容器
			line.points=linePoints;//将points指向linePoints,通过改变points来改变linePoints
			//GridLineTraversal::gridLine(p0, p2, &line);
			GridLineTraversal::gridLine(p0, p1, &line);//获取激光束的所以点
			for (int i=0; i<line.num_points-1; i++){
				activeArea.insert(map.storage().patchIndexes(linePoints[i]));//插入地图内
			}
			if (d<=m_usableRange){//在有效距离内,存储最后一个点
				activeArea.insert(map.storage().patchIndexes(p1));
				//activeArea.insert(map.storage().patchIndexes(p2));
			}
		} else {
			if (*r>m_laserMaxRange||*r>m_usableRange) continue;//如果超出有效范围则退出
			Point phit=lp;//获取机器人坐标
			phit.x+=*r*cos(lp.theta+*angle);//获取点云坐标
...
利用最近一次的观测来提高Proposal distribution进行优化。
...
double ScanMatcher::optimize(OrientedPoint& pnew, const ScanMatcherMap& map, const OrientedPoint& init, const double* readings) const{
	double bestScore=-1;//初始化做高分值
	OrientedPoint currentPose=init;//当以机器人位姿设为前位姿 

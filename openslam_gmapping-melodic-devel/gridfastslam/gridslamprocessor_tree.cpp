...
void  GridSlamProcessor::updateTreeWeights(bool weightsAlreadyNormalized){
  ...
  if (!weightsAlreadyNormalized) {
    normalize();//权重归一化,即是每个粒子权重除以总权重,并计算neff值
  }
  resetTree();//初始化树
  propagateWeights();//返回总权重
}
...
double propagateWeight(GridSlamProcessor::TNode* n, double weight){
	if (!n)
		return weight;
	double w=0;
	n->visitCounter++;//访问次数加一
	n->accWeight+=weight;//权重累加
	if (n->visitCounter==n->childs){
		w=propagateWeight(n->parent,n->accWeight);//意思是父节点的权重是树下所有子节点权重相加
	}
...
double GridSlamProcessor::propagateWeights(){
  // don't calls this function directly, use updateTreeWeights(..) !
	...
	std::vector<double>::iterator w = m_weights.begin();//读取第一地址权重
	for (ParticleVector::iterator it = m_particles.begin(); it != m_particles.end(); it++){
		double weight=*w;//读取权重
		aw+=weight;//权重累加
		TNode * n=it->node;//读取 节点到n
		n->accWeight=weight;//读取权重到节点n
	...
	return lastNodeWeight;//根据树节点,返回节点总权重
}
